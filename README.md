...
### 3.7 非ラーメン店の除外（口コミベース）

`app/services/ramen_search.py::_should_exclude_non_ramen_shop` で、候補店の除外判定を行います。

- 先に「ラーメンシグナル（店名 / type / 概要 / 口コミ / メニュー / 写真説明）」が1つでもあれば除外しない
- それが無い場合、口コミ本文中の非ラーメンキーワード件数をカウント
- **非ラーメンキーワードが 3件以上 かつ ラーメンキーワード 0件** のとき除外
- 上記に該当しない場合は、店名・type・概要の非ラーメンシグナルを見て最終判定
